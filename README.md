...
## ИИ-агент (разбор запроса на этапы)

Главная страница — это ИИ-агент (он же `/assistant`), работает **без регистрации**:
пользователь описывает задачу словами (например, «Монтаж погреба 3×2×2» или «Забор из
профлиста 30 метров на сваях»), агент понимает запрос, разбивает его на этапы и показывает
**две цены** — «по этапам» (через платформу) и «под ключ» — плюс объяснение, **почему по
этапам дешевле**. Регистрация предлагается только в конце, когда пользователь хочет
запустить проект (эндпоинт `POST /api/ai/analyze`).

Цены берутся **гибридно**:

- есть готовый сценарий в БД (погреб) → **точный расчёт** формулами и прайсами
  (`Formula`/`FormulaVersion`, `RegionalPrice`) — числа не выдумываются;
- сценария нет (заборы, навесы и т.п.) → ИИ даёт **ориентировочную оценку** диапазонами
  с явной пометкой «оценка ИИ».

Провайдер понимания выбирается через `AI_DRIVER`:

- `heuristic` (по умолчанию) — офлайн-разбор по ключевым словам и габаритам, **без затрат**;
  подбирает готовый сценарий из БД, но не оценивает цены для произвольных запросов;
- `openrouter` — бесплатные модели [OpenRouter](https://openrouter.ai) (`AI_MODEL`,
  ключ `OPENROUTER_API_KEY`); даёт ориентировочные оценки для любых запросов, при
  сбое/таймауте — мягкий откат на эвристику.
