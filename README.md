...
Страница `/assistant` (в меню — «ИИ-агент») работает **без регистрации**: пользователь
описывает задачу словами (например, «Монтаж погреба 3×2×2»), агент понимает запрос,
подбирает подходящий сценарий, разбивает его на этапы и показывает **две оценки** —
«по этапам» (через платформу) и «под ключ». Регистрация предлагается только в конце,
когда пользователь хочет запустить проект.

Соблюдается ключевой принцип: ИИ только **понимает** запрос и извлекает числовые
параметры; все суммы и количества считаются формулами и прайсами БД (read-only,
эндпоинт `POST /api/ai/analyze`).
...
- `openrouter` — бесплатные модели [OpenRouter](https://openrouter.ai) (`AI_MODEL`,
  ключ `OPENROUTER_API_KEY`); при сбое/таймауте — мягкий откат на эвристику.
